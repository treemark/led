...
public class LedMappingSession implements CommandLineRunner {
    ...
    private void savePositions() {
        try {
            String filename = "led_positions_" + System.currentTimeMillis() + ".js";
            try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
                writer.println("// LED positions detected by mapping session");
                writer.println("// Detected: " + detectedPositions.size() + " of " + totalLeds + " LEDs");
                writer.println("const ledPositions = [");
                
                for (int i = 0; i < totalLeds; i++) {
                    Point p = detectedPositions.get(i);
                    if (p != null) {
                        writer.printf("  [%.1f, %.1f]", p.x, p.y);
                    } else {
                        writer.print("  null");
                    }
                    if (i < totalLeds - 1) writer.print(",");
                    writer.println(" // " + i);
                }
                
                writer.println("];");
            }
            logger.info("Positions saved to {}", filename);
        } catch (IOException e) {
